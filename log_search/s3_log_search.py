		...
		for j in range(num_objects):
			if temp_string.find('\n') > -1:
				...
				if user_object['uuid'] == result_object['value']['uuid']:
					...
					result_object['updated_paths']=user_object['lastUpdated']
...
def lookup_site_name(client_id):
	site_list = {
		...
		"fn78qanvyvndggfhwdsjb9gg7wwzb5yz":"app-owner",
		"kb7fnzc3zjpnrjfxts3ec25m3ktwaf4t":"CNHI",
		"t45xr2ewv345nmmyznpm24u76ktkhwdw":"livefyre",
		"bcmsbu6qranf5dawfw87nkk8dsh35x7b":"login-client",
		...
		"5wegfxdbreywskakdfnkwpc8efd643et":"what-counts"
	}
...
def build_update_object(s3_result_set):
	...
	if len(s3_result_set) > 0:
		...
		for result in s3_result_set:			
			...
			print result
			try:
				update_object['target_fields_paths']=result['audit']['authoritative']['target_fields_paths']
			except:
				update_object['target_fields_paths']=result['attributes']
			update_object['uuid'] = result['value']['uuid']
			update_object['site_name']=lookup_site_name(result['client_id'])
			update_object['login_client']=result['client_id']
...
def main():
	...
	global users_file
	global json_file
	# users_file = open('target_users.json','r')
	...
	with open('target_users.json','r') as f:
